...
func (s *Server) handleRefreshToken(w http.ResponseWriter, r *http.Request, client storage.Client) {
	...
	newToken := token
	if s.refreshTokenPolicy.RotationEnabled() || !s.refreshTokenPolicy.AllowedToReuse(rCtx.storageToken.LastUsed, now) {
		replaceToken, ident, rerr := s.updateRefreshToken(r.Context(), rCtx, now)
		if rerr != nil {
			s.refreshTokenErrHelper(w, rerr)
			return
		}
